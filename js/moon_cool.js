...
// 225,224,229  ->  0.5778,0.0588,1.0000
...
function draw(){
  ...
  push();
  colorMode(RGB);
  ctx.shadowColor = 'rgba(225,224,229,0.8)';
  ctx.shadowBlur = 35;
  fill(225,224,229);//没有填充色的话是不会存在阴影的
  noStroke();
  ...
  for(let i = 0; i<width; ++i){
    for(let j = 0; j<height; ++j ){
      stroke(0.5778,0.0588,constrain(map(noise(i*xMult,j*yMult),0,1,0.55,1.2), 0, 1));
      point(i, j);
    }
  }
  pop();
